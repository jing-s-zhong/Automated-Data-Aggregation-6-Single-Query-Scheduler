...
FROM VALUES (
  ...
  $$,
  'Data Poulation', 'BI-1144: Populate sessions and pageviews summary', true, 220, 2, '20 10/2-19 * * *'
  )
) S
ON D.JOB_LABEL = S.JOB_LABEL
WHEN MATCHED THEN UPDATE SET
...
FROM VALUES (
  ...
  $$,
  'Manual Entry', 'BI-860: Method to add manual entry data for revenue', true, 310, 2, '20 3/3-18 * * *'
  )
) S
ON D.JOB_LABEL = S.JOB_LABEL
WHEN MATCHED THEN UPDATE SET
...
FROM VALUES (
  ...
  USING (
      SELECT A.DATA_TS
        ...
        ,NULL::NUMBER CLICKS
        ,NULL::NUMBER IMPRESSIONS
        ,A.ESTIMATED_REVENUE * 0.45 NATIVE_SPEND
        ,A.ESTIMATED_REVENUE * 0.45 SPEND
  ...
  $$,
  'Manual Entry', 'BI-1029: change thryv spend to come from manual entry', true, 320, 2, '20 4/3-19 * * *'
  )
) S
ON D.JOB_LABEL = S.JOB_LABEL
WHEN MATCHED THEN UPDATE SET
