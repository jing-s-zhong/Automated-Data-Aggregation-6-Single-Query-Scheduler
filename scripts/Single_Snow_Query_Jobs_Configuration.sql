--#########################################################
-- DATA PROCESS: SINGLE SNOW QUERY JOBS CONFIGURATION
--#########################################################
--
-- This section is to accept snowSQL client params. skip it if run this script manually
--
...
-----------------------------------------------------------
--
-- If run script manually in web UI, set the environment params following this section
--
  ...
  USING (
    WITH G_SHEET AS (
      SELECT A.DATE DATA_TS
        ...
        ,QUORA_SPEND_USD NATIVE_SPEND
        ,QUORA_SPEND_USD SPEND
      FROM BI.MANUAL_ENTRY.PUBLISHING_MANUAL_SPEND_DAILY A
    ...
    FULL JOIN (
      SELECT DATA_TS
          ,ACCOUNT_ID
          ,NETWORK_NAME_ID
      FROM MANUAL_ENTRY.BUYSIDE_ACCOUNT_DATA_MANUAL_ENTRY A
      JOIN (
          SELECT ACCOUNT_ID, NETWORK_NAME_ID
          FROM G_SHEET
          GROUP BY 1,2
          ) B
      USING (ACCOUNT_ID,NETWORK_NAME_ID)
      WHERE DATA_TS >= '2020-01-01'
      ) E
    USING (DATA_TS,ACCOUNT_ID,NETWORK_NAME_ID)
    ) S
  ON D.DATA_TS = S.DATA_TS
    AND D.NETWORK_NAME_ID = S.NETWORK_NAME_ID
    AND D.ACCOUNT_ID = S.ACCOUNT_ID
  WHEN MATCHED AND S.NETWORK_NAME IS NULL
    THEN DELETE
  ...
  USING (
    WITH G_SHEET AS (
      SELECT A.DATE DATA_TS
        ...
        ,NULL::NUMBER CLICKS
        ,NULL::NUMBER IMPRESSIONS
        ,APPLENEWS_SPEND_USD NATIVE_SPEND
        ,APPLENEWS_SPEND_USD SPEND
      FROM BI.MANUAL_ENTRY.PUBLISHING_MANUAL_SPEND_DAILY A
      JOIN BI.COMMON.NETWORK_NAMES B
  ...
  USING (
    WITH G_SHEET AS (
      SELECT A.DATE DATA_TS
        ...
        ,NULL::NUMBER CLICKS
        ,NULL::NUMBER IMPRESSIONS
        ,PLAYANEXT_SPEND NATIVE_SPEND
        ,PLAYANEXT_SPEND SPEND
      FROM BI.MANUAL_ENTRY.WATERFOX_SPEND A
      JOIN BI.COMMON.NETWORK_NAMES B
  ...
  USING (
    WITH G_SHEET AS (
      SELECT A.DATE DATA_TS
        ...
        ,CNN_SPEND_USD NATIVE_SPEND
        ,CNN_SPEND_USD SPEND
      FROM BI.MANUAL_ENTRY.GINSU_MANUAL_SPEND_DAILY A
      JOIN BI.COMMON.NETWORK_NAMES B
        ON B.NETWORK_NAME = 'CNN'
      )
    SELECT *
  ...
  USING (
    WITH G_SHEET AS (
      SELECT A.DATE DATA_TS
        ...
        ,NULL::NUMBER CLICKS
        ,NULL::NUMBER IMPRESSIONS
        ,CRITEO_SPEND NATIVE_SPEND
        ,CRITEO_SPEND SPEND
      FROM BI.MANUAL_ENTRY.SHOPPING_MANUAL_ENTRY A
      JOIN BI.COMMON.NETWORK_NAMES B
...
FROM VALUES (
  ...
  USING (
    ...
    FROM BI.MANUAL_ENTRY.SELLSIDE_MAPQUEST_CLICKTRIPZ_PERFORMANCE_DAILY
